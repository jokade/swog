...
//        val transformedBody = genTransformedCompanionBody(obj) ++ obj.data.additionalCompanionStmts :+ genBindingsObject(obj.data)
        val transformedBody = genTransformedCompanionBody(obj) ++ obj.data.additionalCompanionStmts
    ...
    override protected def genTransformedCompanionBody(t: TransformData[CommonParts]): Seq[c.universe.Tree] = {
      val updBody =
        t.modParts.body.map{
          ...
          case m @ DefDef(mods,name,tparams,args,tpe,rhs) if executeSyncOnMainThread(m) =>
            val updRhs = q"scalanative.interop.Dispatch.onMainThread(() => $rhs, true)"
            DefDef(mods,name,tparams,args,tpe,updRhs)
          case x => x
        }
